# Adendo da interface AUTOPACKLINE x CLP - Rev. 03

Data: 18/09/2026

Este documento substitui o adendo Rev.02. Ele congela o contrato para o
desenvolvimento offline do AUTOPACKLINE e identifica o que ja foi implementado na
Rev.06 do Ladder e o que ainda depende do comissionamento.
...
| Revisao do Ladder | 6 em D764 |
| Data da revisao | 18/09/2026, representada por D765=2026 e D766=918 |
| Arquivo de referencia | `CLP_COMAU_rev06_scanner_full.MPU` |
...
|---|---|---|
| D764 | AP_LADDER_REVISION | Revisao logica do Ladder; valor 6 nesta entrega |
| D765 | AP_LADDER_BUILD_YEAR | Ano da revisao; valor 2026 |
| D766 | AP_LADDER_BUILD_MMDD | Mes e dia sem separador; valor 918 para 18/09 |
...
| D771 | AP_READER_DATA_LENGTH | Comprimento do dado bruto em bytes, 0 a 82 |
...
| D779 | AP_READER_BLOCK_LENGTH | Tamanho do bloco do leitor em words; valor 89 |
...
| 3 | AP_FEATURE_READER_RAW | D800..D888 contem dado bruto valido do SR-1000 |
| 4 | AP_FEATURE_READER_PARSED | Campos serial/EAN/OP/modelo foram separados pelo CLP |

Na Rev.06, D777=15. Dashboard, reteste, status e dado bruto do leitor estao
implementados. O bit de dado analisado permanece zerado: o AUTOPACKLINE deve
separar os campos a partir do texto bruto.

## 3. Bloco do SR-1000

O bloco do leitor fica separado em D800..D888 para nao reduzir os tamanhos dos
...
| D846 | READER_RAW_LENGTH | UINT16 | 0 a 82 caracteres |
| D847..D887 | READER_RAW_DATA | ASCII[82] | Conteudo bruto completo lido pelo SR-1000 |
| D888 | READER_DATA_ERROR | UINT16 | Codigo de erro associado a leitura |
...
O teste Modbus da Rev.06 em 18/09/2026 confirmou AB12. Os 82 bytes foram
reconstruidos integralmente, incluindo `https://www.elgin.com.br` e CR final.
Bibliotecas que apresentem bytes invertidos devem aplicar a conversao para AB12.
...
| Enderecos canonicos | Dispositivos Delta D700..D888 |
...
2. O CLP publica o resultado e o dado bruto em D767..D888.
...
- simulador dos blocos D700..D779 e D800..D888;
...
confirmou 8 POUs, 325 redes, preservacao do mapa D100/D200 e APIs compativeis com
o AS228T. A Rev.06 foi compilada, baixada e teve uma leitura boa de 82 bytes
validada em 18/09/2026. Permanecem obrigatorios:

1. comparar o programa online com a base usada na Rev.06;
2. validar um no read e a recuperacao com uma nova leitura boa;
3. validar heartbeat, ACK/NACK e reteste do AUTOPACKLINE;
4. somente depois habilitar o inicio de ciclo pelo comando do AUTOPACKLINE.
