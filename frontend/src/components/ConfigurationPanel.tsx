...
import { createLine, createProduct, listIndicatorThresholds, setIndicatorThreshold, setPalletConfig, setProductionTarget } from '../services/api'
...
export function ConfigurationPanel({ products, lines, configs, targets, onChanged }: Props) {
  ...
  const [openPanel, setOpenPanel] = useState<'product' | 'line' | 'pallet' | 'target' | 'alerts' | 'alertconfigs' | 'targets' | 'configs'>('target')
  const configPageSize = 5
  const targetPageSize = 5
  const [productForm, setProductForm] = useState({
    sku: '', ean: '', model: '', name: '', capacity_btu: '', active: true,
  })
  ...
  async function handleProduct(event: FormEvent) {
    ...
    try {
      ...
      setProductForm({ sku: '', ean: '', model: '', name: '', capacity_btu: '', active: true })
