...
from .rev03_contract import decode_features, decode_reader_snapshot
...
class ExternalSimulatorConfig:
    ...
    def address_for(self, logical_d: int) -> int:
        if not 700 <= logical_d <= 888:
            raise ValueError("registrador lógico fora do contrato D700-D888")
...
class ExternalPlcSimulatorAdapter:
    adapter = "EXTERNAL_CLP_SIMULATOR_MODBUS_TCP_REV03"
    ...
    def diagnostic(self) -> dict:
        ...
        return {
            ...
            "logical_map": {
                ...
                "D800-D888_reader": f"holding {self.config.address_for(800)}-{self.config.address_for(888)}",
            },
    ...
    def probe(self, client: ModbusTcpClient | None = None) -> dict:
        ...
        try:
            ...
            expected_identity = {750: 1, 764: 6, 765: 2026, 766: 918, 777: 15, 778: 800, 779: 89}
            ...
            if mismatches:
                raise ValueError(f"identidade Rev.03 incompatível: {mismatches}")
            result = {
            ...
            if result["features"]["reader_raw_valid"]:
                reader_values = transport.read_holding_registers(self.config.address_for(800), 89)
